...
function setup() {
  ...
  score=0;
  gameState="start";
  welcome_screen=createSprite(width/2,height/2,width,height)
  welcome_screen.addImage(welcome_img);
  welcome_screen.scale=2.5;

  button=createSprite(width/2,500,200,200);
  ...
  bulletgrp=new Group();
   ...
   lose=createSprite(width/2,height/2-250,50,50);
   lose.addImage(lose_img)
   lose.scale=0.5;
   lose.visible=false;

   house =createSprite(width,height/2+90,50,50)
    house.addImage(house_img);
    house.scale=0.5;
    house.visible=false;

    win=createSprite(width/2-30,height/2-80,50,50);
    win.addImage(win_img)
    win.scale=2.28;
    win.visible=false;

    retry=createSprite(width/2,510);
...
if(gameState==="start"){
  ...
  if(gameState==="end"){
    ...
    fill("red")
    stroke("yellow")
    //text("You were not able to save the citizens!",width/2-400,325);
    //text("           Press retry for another go.",width/2-400,375);
  ...
  if(gameState==="win"){
    ...
    fill("blue")
    stroke("aqua")
    text("  You saved the citizens!",width/2-650,550);
    text("Press replay to play again.",width/2-650,600);
...
function spawnObstacles(){
  if(frameCount % 40===0){
   bullet=createSprite(width,random(20,height-50),40,40)
